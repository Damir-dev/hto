...
/*Фильтр для A-элементов - проверить есть ли переполнение, если осталось, пропустить через фильтр элементы А*/
function aElementsFilter(firstarray) {
	var arrayWithAElements = new Array();
	var arrayWithChildrenAElements = new Array();
	var fourtharray = new Array();
	for (var i = 0; i < firstarray.length; i++) {
		if (firstarray[i].nodeName == "A") {
			arrayWithAElements.push(firstarray[i]);
		}
	}
	for (var i = 0; i < arrayWithAElements.length; i++) {
		if (arrayWithAElements[i].childNodes.length > 0) {
			for (var j = 0; j < arrayWithAElements[i].childNodes.length; j++) {
				arrayWithChildrenAElements.push(arrayWithAElements[i].childNodes[j]);
			}
		}
	}
	for (var i = 0; i < arrayWithChildrenAElements.length; i++) {
		var objectContainer = new HtoElement(i, arrayWithChildrenAElements[i].nodeValue, arrayWithChildrenAElements[i]);
		fourtharray.push(objectContainer);
		arrayWithChildrenAElements[i].nodeValue = "";
	}
	return fourtharray;
}
...
function hideTextOverflow(htoLineCounter, crop) {
	...
	var tempArray = aElementsFilter(firstarray);
	for (var i = 0; i < tempArray.length; i++) {
		fourtharray.push(tempArray[i]);
	}
